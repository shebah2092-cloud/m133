...
def load_sitl_csv(path: Path) -> pd.DataFrame:
    ...
    with np.errstate(divide='ignore', invalid='ignore'):
        vz_ned = df["vel_z"]  # positive = up in ECEF, but check sign
        v_h = df["speed_horizontal"]
        # gamma = arctan(Vup / Vh) — need to determine sign convention
        # In the SITL bridge, vel is in ECEF. altitude increases = going up.
        # Let's derive from altitude change
...
def score_run(m: dict) -> dict:
    ...
    # Range accuracy (absolute error in meters, gradual scoring)
    w = W["range"]
    ...
    # Stability (pitch std in last 30%)
    w = W["stability"]
